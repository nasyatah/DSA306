...
PRICE AND QUANTITY ANALYSIS OF TOP PPRODUCTS USING LOGISTIC REGRESSION
...
#creating dummy variable to segment top product and not top_product

#top_products_spark <- copy_to(sc, top_products) #create a Spark Dataframe of top_products

top_product_analysis <- transaction_clean |>
    mutate(isTopProduct = ifelse(transaction_clean$ProductNo %in% top_products$ProductNo, 1, 0)) |>
    group_by(ProductName)|>
    dplyr::mutate(AvgTransactionQuantity = mean(Quantity))  #creating a dummy variable

head(top_product_analysis)

#running logistic regression on Price and Quantity

top_producstc_logistic_regression <- top_products_analysis|>
  ml_logistic_regression(formula = isTopProduct ~ Price + AverageTransactionQuanity) |> 
  tidy() 

#creating training and testing set for 10-fold cross-validation

top_prosucts_analysis_split <- top_product_analysis|>
  sdf_random_split(training = 0.8, tetsing = 0.2, seed = 123)

top_product_analysis_train <- top_prosucts_analysis_split$training

top_product_analysis_test <- top_prosucts_analysis_split$testing

folds <- sdf_random_split(
  top_product_analysis_train,
  weights = setNames(rep(0.1,10), paste0("fold",1:10)),
  seed = 123
)

#create first training and assesment set

analysis_set <- do.cll("rbind", folds[2:10])
assesment_set <- folds[[1]]

#creating our trained model

lr_fit <- analyis_set|>
  ml_logistic_regression(formula = isTopProduct ~ Price + Quanity
    formulas
  )

lr_fit

#assesing out train model

lr_fit_summary<- ml_evaluate(lr_fit, dataset = assesment_set)

lr_fit_summary$areaunderROC
```
