...
ref_customer <- transaction_clean |>
  group_by(CustomerNo) |>
  summarise(
    Recency = as.numeric(datediff(max(FormattedDate), to_date("2019-09-09"))*(-1)),
...
temp_df <- transaction_clean |> 
  ...
  rename(month_with_max_spending = "month(FormattedDate)",
         amt_spent_during_mth_w_max_spending = total_spent_per_month)
...
#create a column for average basket size per customer
