...
transaction_ref_product <- transaction_clean |>
  mutate(Revenue = Price * Quantity) # Create a 'Revenue' column
...
product_rfm <- transaction_ref_product |>
  ...
  summarise(
    ...
    Frequency = n(), # Number of transactions the product appears in
...
revenue_threshold <- sum(product_rfm$CumulativeRevenue) * 0.8 # Set a revenue cutoff
