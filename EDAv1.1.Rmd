...
```{r}
transaction_ref_customer <- transaction_clean |>
  group_by(CustomerNo) |>
  summarise(
...
rfm_stats <- transaction_ref_customer |>
  summarize(
    r_mean = mean(Recency), 
    r_sd = sd(Recency), 
...
transaction_ref_customer1 <- transaction_ref_customer |>
...
transaction_ref_customer_split <- transaction_ref_customer |>  
  sdf_random_split(training = 0.8, testing = 0.2, seed = 44)

transaction_ref_customer_split_train <- transaction_ref_customer_split$training
transaction_ref_customer_split_test <- transaction_ref_customer_split$testing

fit_1 <- transaction_ref_customer_split_train |> 
  ml_linear_regression(formula = Monetary ~ Recency)

fit_2 <- transaction_ref_customer_split_train |> 
  ml_linear_regression(formula = Monetary ~ Frequency)

fit_3 <- transaction_ref_customer_split_train |> 
  ml_linear_regression(formula = Monetary ~ Frequency + Recency) 

fit_4 <- transaction_ref_customer_split_train |> 
...
pred_1 <- ml_predict(fit_3, dataset = transaction_ref_customer_split_test)
...
pred_2 <- ml_predict(fit_4, dataset = transaction_ref_customer_split_test)
...
library(rgl)
x <- collect(transaction_ref_customer_split_train$M_standardized)
y <- collect(transaction_ref_customer_split_train$R_standardized)
z <- collect(transaction_ref_customer_split_train$F_standardized)
transaction_customer_train |> plot3d( x,y,z, col = "blue", xlab = "Monetary", ylab = "Recency", zlab = "Frequency", main = "3D Plot")
...
transaction_ref_product <- transaction_clean |>
  mutate(Revenue = Price * Quantity) #creating revenue column

product_rfm <- transaction_ref_product |>
  group_by(ProductNo, ProductName) |>
  summarise(
    Recency = as.numeric(datediff(to_date("2019-09-12"), max(FormattedDate))),
    Frequency = n(), #num of transactions the product appears in
...
product_rfm <- product_rfm |>
   arrange(desc(Monetary)) |>
   mutate(CumulativeRevenue = cumsum(Monetary)) |> collect()

revenue_threshold <- sum(product_rfm$CumulativeRevenue) * 0.8 #cutoff
 
top_products <- product_rfm |>
  filter(Monetary <= revenue_threshold) |>
  arrange(desc(Monetary))

#Visualize the Pareto distribution of top products
dbplot_bar(top_products, x = ProductName, y = CumulativeRevenue) +
   labs(title="Top Products by Revenue", x="Product Name", y="Revenue")
 
##after visualisation, recognition of pareto distribution - below are tests to confirm
