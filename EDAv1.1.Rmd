...
```{r}
date_summary <- transaction_clean |>
  summarize(
    min_date = min(FormattedDate),
    max_date = max(FormattedDate)
  ) |>
  collect()
...
#make month with max spending a categorical variable
ref_customer <- ref_customer |>
  mutate(month_with_max_spending = as.character(month_with_max_spending))
...
#Confidence Interval for the features
```{r}
tidy_glm_fit <- ref_customer_split_train |>
  ml_generalized_linear_regression(
    formula = logistic_duration ~ Recency + Frequency + Monetary + Average_basket_size + Unique_products + highest_month_spending + month_with_max_spending, family = "binomial"
  ) |>
  tidy() |>
  collect()
...
```{r}
#month with max spending - change to festive or not festive 
#average basket size eda
fit_logistic2 <- ref_customer_split_train |>
  ml_generalized_linear_regression(formula = logistic_duration ~ M_standardized + F_standardized + R_standardized + Avg_spend_per_transaction + Unique_products + month_with_max_spending, family = "binomial")
...
fit_logistic3 <- ref_customer_split_train |>
  ml_logistic_regression(formula = logistic_duration ~ M_standardized + F_standardized + R_standardized + Average_basket_size + Unique_products + month_with_max_spending)
