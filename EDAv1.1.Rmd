...
reg <- lm(Monetary ~ Recency + Frequency, data = transaction_ref3)
...
popular_product <- transaction_ref2 |> 
...
#the average interval between sales of product --> frequency over a period time


#product LTV = product monetary value x frequency 
...
fig <- plot_ly(busy_dates, type = 'scatter', mode = 'lines')%>%
  add_trace(x = ~FormattedDate, y = ~Total_transactions)%>%
...
#in that particular month --> what is the compoiton of products --> which is the top 5 and bottom 5 
