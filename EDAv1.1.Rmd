...
ref_customer <- transaction_clean |>
  group_by(CustomerNo) |>
  summarise(
    Recency = as.numeric(datediff(
      max(FormattedDate), 
      to_date("2019-12-31"))*(-1)),
    ...
    Avg_time_btwn_orders = (mean(as.numeric(diff(FormattedDate))))
    ) |>
    mutate(Avg_spend_per_transaction = Monetary/Recency)
 
ref_customer
ref_customer <- ref_customer |>
  mutate(logistic_duration = ifelse(Duration > 180, 1, 0),
         Avg_spend_per_transaction = (Monetary/Recency))
