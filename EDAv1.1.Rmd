...
top_products_spark <- copy_to(sc,top_products)

product_regression_data <- transaction_clean |>
    group_by(ProductNo)|>
  dplyr::summarise(
    Price = mean(Price),
    Frequency = n(),
    AvgTransactionQuantity = mean(Quantity),
    isTopProduct = ifelse(transaction_clean$ProductNo %in% top_products_spark$ProductNo, 1, 0)) 

product_regression_data_renv <- product_regression_data |> collect()

product_regression_data |> sdf_describe(cols = c("isTopProduct","Price","Frequency","AvgTransactionQuantity"))

#running logistic regression on Price and Quantity
top_products_logistic_regression <- product_regression_data|>
  ml_logistic_regression(formula = isTopProduct ~ Price + AvgTransactionQuanity + Frequency)|>tidy()

#creating training and testing set for 10-fold cross-validation

top_prosucts_analysis_split <- regression_|>
