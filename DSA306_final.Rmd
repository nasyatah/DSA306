...
#Remove average basket size since it is highly correlated with average spend per transaction
```
...
5.1.3 Standardisation of variables (i think we shld split test/train before standardising?)
```{r}
#standardised values
...
ref_customer_stats <- ref_customer_split_train |>
  summarize(
    ...
    Avg_spend_per_transaction_mean = mean(Avg_spend_per_transaction), Avg_spend_per_transaction_sd = sd(Avg_spend_per_transaction)
...
ref_customer_split_train <- ref_customer_split_train |>
  mutate(R_standardized = (Recency - !!ref_customer_stats$r_mean) / !!ref_customer_stats$r_sd,
         ...
         Avg_spend_standardized = (Avg_spend_per_transaction - !!ref_customer_stats$Avg_spend_per_transaction_mean) / !!ref_customer_stats$Avg_spend_per_transaction_sd) 
...
ref_customer_split_test <- ref_customer_split_test |>
  mutate(R_standardized = (Recency - !!ref_customer_stats$r_mean) / !!ref_customer_stats$r_sd,
         ...
         Avg_spend_standardized = (Avg_spend_per_transaction - !!ref_customer_stats$Avg_spend_per_transaction_mean) / !!ref_customer_stats$Avg_spend_per_transaction_sd) 
ref_customer_split_train |>
  sdf_describe(cols = c("R_standardized","F_standardized","M_standardized","Unique_products_standardized","Avg_spend_standardized"))

```

5.1.x CI plot - with all the variables 
```{r}
...
validation_summary <- ml_evaluate(fit_logistic1, dataset = ref_customer_split_test)
...
#fit_logistic3$summary$area_under_roc() 
validation_summary3 <- ml_evaluate(fit_logistic3, dataset = ref_customer_split_test)
validation_summary3$area_under_roc()

#rfm + festive spender
fit_logistic4 <- ref_customer_split_train |>
  ml_logistic_regression(formula = logistic_duration ~ M_standardized + F_standardized + R_standardized + festive_spender)

#fit_logistic4$summary$area_under_roc() 
validation_summary4 <- ml_evaluate(fit_logistic4, dataset = ref_customer_split_test)
validation_summary4$area_under_roc()
...
pred_3 <- ml_predict(fit_logistic3, dataset = ref_customer_split_test)
MSE_3<- ml_regression_evaluator(
pred_3,
label_col = "logistic_duration",
prediction_col = "prediction",
metric_name = "mse"
...
pred_4 <- ml_predict(fit_logistic4, dataset = ref_customer_split_test)
MSE_4<- ml_regression_evaluator(
pred_4,
label_col = "logistic_duration",
prediction_col = "prediction",
metric_name = "mse"
